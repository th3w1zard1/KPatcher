...
namespace KPatcher.Core.Tests.Logger
{
    ...
    {
        [Fact]
        public void AddDiagnosticShouldAddLogWithDiagnosticType()
        {
            var logger = new PatchLogger();

            logger.AddDiagnostic("diag line");

            logger.AllLogs.Should().HaveCount(1);
            logger.AllLogs[0].LogType.Should().Be(LogType.Diagnostic);
            logger.Diagnostics.Should().HaveCount(1);
        }
        ...
        [Fact]
        public void DiagnosticLoggedEventShouldFireWhenDiagnosticAdded()
        {
            var logger = new PatchLogger();
            PatchLog received = null;
            logger.DiagnosticLogged += (sender, log) => received = log;

            logger.AddDiagnostic("x");

            received.Should().NotBeNull();
            received.LogType.Should().Be(LogType.Diagnostic);
        }
