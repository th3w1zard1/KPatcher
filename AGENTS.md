...
| Run tests | **Mandatory:** `.\scripts\DotnetTest.ps1 KPatcher.sln -c Debug` (Windows) or `./scripts/dotnet-test.sh KPatcher.sln -c Debug` (Unix). **Do not** invoke bare `dotnet test` in agents, CI, or automation. Run the wrapper **once** per verification; **do not** poll or wait hours — if a run hits **124** (timeout) or otherwise fails to finish in one invocation, **stop and find the bottleneck**. **Never exceed 10 minutes** wall clock: `-TimeoutSeconds` / `DOTNET_TEST_TIMEOUT_SECONDS` are **capped at 600** (default **600**). **`KPatcher.Tests`** applies **`tests/KPatcher.Tests/Default.runsettings`**, which excludes **`Category=DeNCSRoundTrip`** (exhaustive `NCSDecompCliRoundTripTest`) so default runs finish **well under** the cap. To run only that harness locally: `--settings tests/KPatcher.Tests/Exhaustive.runsettings` (long-running; often outside the wrapper). On **124**, **mandatory bottleneck work** — do **not** satisfy timeouts by commenting out, skipping, or disabling tests as the primary fix. |
...
- **`NCSDecompCliRoundTripTest`** (traits `ExternalCompiler` / `Vendor` / `DeNCSRoundTrip`): exhaustive vanilla NSS↔NCS harness (external **`nwnnsscomp`**, managed decompile, etc.). **Excluded from default runs** via **`Default.runsettings`** (`Category!=DeNCSRoundTrip`) so CI/agents stay under the 10-minute wrapper cap; use **`Exhaustive.runsettings`** to run it locally when needed. Default NSS/NCS verification should rely on **managed** **`KCompiler`** + **`NCSDecomp.Core`** tests per **`/lfg`**. **Do not** raise the wrapper cap or blanket-disable tests as the workaround — **optimize or shard** the exhaustive job if it must return to default CI.
...
- **`dotnet test`**: **Only** via `scripts/DotnetTest.ps1` or `scripts/dotnet-test.sh` (**never** bare `dotnet test` for agents/CI). One invocation per check; **no** long polling. Wall clock **max 600s** (default **600**). On **124**, **find the bottleneck**; do not disable tests. **`Default.runsettings`** on `KPatcher.Tests` excludes exhaustive `DeNCSRoundTrip` by default.
