...
| Run tests | **Always** use the timeout wrapper (kills `dotnet` when time expires): `.\scripts\DotnetTest.ps1 KPatcher.sln -c Debug` (Windows PowerShell) or `./scripts/dotnet-test.sh KPatcher.sln -c Debug` (Unix). **Never exceed 10 minutes total** wall clock for any `dotnet test` run: `-TimeoutSeconds` and `DOTNET_TEST_TIMEOUT_SECONDS` are **capped at 600 seconds**; default is **600**. The suite must complete **well under** 10 minutes in normal practice — if not, **find the bottleneck** (profiling, hot paths, test partitioning, parallelism, caching) and fix it. Exit **124** = timeout — **mandatory bottleneck investigation**; do **not** satisfy timeouts by commenting out, skipping, or disabling tests. Do **not** run bare `dotnet test` from agents/automation. |
...
- **`NCSDecompCliRoundTripTest`** (traits `ExternalCompiler` / `Vendor` / `DeNCSRoundTrip`): exhaustive vanilla NSS↔NCS checks (optional **`nwnnsscomp.exe`**, managed decompile, KCompiler parity). It needs **`include/*.nss`** and **`Vanilla_KOTOR_Script_Source`** when exercised. The **wrapper never allows more than 10 minutes** for the whole `dotnet test` process — the full exhaustive suite **must be split, accelerated, or run in smaller CI shards** so a single wrapper invocation finishes **well under** 10 minutes; **find bottlenecks** (I/O, compiler invocations, decompiler hot paths). **Do not** raise the cap or blanket-disable tests as the workaround. For smaller managed-only checks, use **`NCSRoundtripTests`** / **`VanillaNSSCompileTests`** where appropriate.
...
- **`dotnet test`**: Use `scripts/DotnetTest.ps1` or `scripts/dotnet-test.sh` so the test process is terminated after a wall-clock limit (**max 600s / 10 minutes total, never more**; default **600**). On timeout (**124**), **find the bottleneck** and optimize until runs complete **way sooner** than 10 minutes; do not disable tests. Avoid raw `dotnet test` in agent-driven runs.
