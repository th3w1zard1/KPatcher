...
| Run tests | **Always** use the timeout wrapper (kills `dotnet` when time expires): `.\scripts\DotnetTest.ps1 KPatcher.sln -c Debug` (Windows PowerShell) or `./scripts/dotnet-test.sh KPatcher.sln -c Debug` (Unix). Optional leading `-TimeoutSeconds N` and env `DOTNET_TEST_TIMEOUT_SECONDS` are **capped at 300 seconds (5 minutes)**; default is **300**. Exit **124** = timeout — **profile and optimize** the slow suite or code path; do **not** satisfy timeouts by commenting out, skipping, or disabling tests. Do **not** run bare `dotnet test` from agents/automation. |
...
- **`NCSDecompCliRoundTripTest`** (traits `ExternalCompiler` / `Vendor` / `DeNCSRoundTrip`): exhaustive vanilla NSS↔NCS checks (optional **`nwnnsscomp.exe`**, managed decompile, KCompiler parity). It needs **`include/*.nss`** and **`Vanilla_KOTOR_Script_Source`** when exercised. The **wrapper hard-limits wall clock to 5 minutes** for the whole `dotnet test` process — if the suite cannot finish, **shrink scope per run** (filters/traits), **parallelize**, **cache artifacts**, or **speed up** compile/decompile paths; **do not** raise the cap or blanket-disable tests as the workaround. For smaller managed-only checks, use **`NCSRoundtripTests`** / **`VanillaNSSCompileTests`** where appropriate.
...
- **`dotnet test`**: Use `scripts/DotnetTest.ps1` or `scripts/dotnet-test.sh` so the test process is terminated after a wall-clock limit (**max 300s**, default **300s**). On timeout, optimize performance; do not disable tests. Avoid raw `dotnet test` in agent-driven runs.
