...
param(
    [string]$TestProjectPath = "tests\KPatcher.Tests\KPatcher.Tests.csproj"
)
...
# Find all test files
$testDir = Split-Path -Parent $TestProjectPath
$testFiles = Get-ChildItem -Path $testDir -Filter "*.cs" -Recurse | Where-Object {
