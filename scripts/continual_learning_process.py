...
def extract_patterns(messages: List[str]) -> Dict[str, List[str]]:
    ...
    for msg in messages:
        ...
        if (
            any(kw in msg_lower for kw in ["meticulous", "thorough", "strict", "assert"])
            and "test" in msg_lower
        ):
        ...
        for sent in sentences:
            ...
            if any(
                oneoff in sent_lower
                for oneoff in ["this file", "this test", "now", "today", "yesterday", "right now"]
            ):
                continue

            # Skip transient details
            if any(
                transient in sent_lower
                for transient in ["branch", "commit", "pr #", "pull request"]
            ):
                continue
            ...
            if has_strong or has_correction:
                ...
                if any(
                    ...
                    generalized = re.sub(
                        r"\b(dumb|fuck|shit|damn)\w*\b", "", generalized, flags=re.IGNORECASE
                    )
...
def main():
    ...
    if not merged["preferences"] and not merged["facts"]:
        ...
        index["transcripts"] = {
            k: v for k, v in index["transcripts"].items() if k in existing_paths
        }
    ...
    print(
        f"Updated AGENTS.md with {len(merged['preferences'])} preferences and {len(merged['facts'])} facts"
    )
