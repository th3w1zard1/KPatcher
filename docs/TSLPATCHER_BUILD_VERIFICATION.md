...
**26 pipeline functions** were created manually (Ghidra was not auto-analyzed; only 254 imported thunks were pre-recognized). The pipeline order table in this document is the authoritative binary RE summary; see [docs/TSLPATCHER_RE.md](TSLPATCHER_RE.md) for the KPatcher mapping index and related characterization tests.
