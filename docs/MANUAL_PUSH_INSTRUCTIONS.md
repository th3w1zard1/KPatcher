...
```powershell
Get-ChildItem "src/KPatcher.Core/bin/Release" -Filter "*.nupkg"
```

## Step 2: Check Package Metadata Version

```powershell
$pkg = "src/KPatcher.Core/bin/Release/KPatcher.Core.0.1.0.nupkg"
...
**Note:** The package filename and the package metadata version should both read `0.1.0` before you publish.
