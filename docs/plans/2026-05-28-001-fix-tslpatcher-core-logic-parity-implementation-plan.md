...
- Landed: additional owner-path parity fixes now cover the binary-verified install queue order, vendor-default HACK write semantics within the existing NCS-backed surface, HACKList skip-on-existing behavior for compiled-script patches unless replacement is explicitly enabled, removal of unsupported top-level `!DefaultSourceFolder` handling in HACKList, vendored HACKList handling that now ignores per-file `!Destination` while preserving per-file `!SourceFile`/`!SaveAs`, vendored HACKList missing-source failure wording for both renamed-source and default-source paths without the later generic lookup error duplication, vendored non-replace HACKList save-as copy/skip note wording in the Override path, CompileList prep/include/failure behavior, removal of unsupported top-level `!DefaultSourceFolder` handling in CompileList, the vendored Override/source-name skip guard for non-replace CompileList entries, managed parsing/application of vendored `ScriptCompilerFlags` for CompileList under the repo's managed-compiler policy, vendor-shaped CompileList include lookup breadth limited to the script directory plus staged `tslpatchdata` root, managed CompileList compiler-feedback surfacing into line-by-line verbose logs with the vendored `NWNNSSComp says:` wrapper for the currently characterized missing-include and entry-point exception paths including the split compiler exception hierarchies behind those cases, vendor-shaped malformed-`ScriptCompilerFlags` feedback that now logs parser output as compiler feedback before the normal skip path, vendor-shaped custom `--nwscript` parse/path failures that now surface as compiler feedback before the normal skip path, explicit Windows code-page registration in the managed NSS path plus its parity tests so Linux CI can exercise the vendor-shaped CompileList slice reliably, vendored compiled-output-not-found wording for the no-NCS-produced path, an explicit managed-compiler-unavailable distinction so backend-load failures no longer masquerade as per-script compile failures, SSF recovery plus 40-slot handling, vendored `!OverrideType` default/rename behavior, and removal of unsupported top-level `!DefaultDestination` handling outside CompileList.
- Partial/uncertain: remaining HACKList follow-up is now limited to any still-uncharacterized per-file source/save-as runtime edge cases beyond the landed top-level-source-folder, per-file-destination, missing-source wording, and non-replace Override copy/skip note fixes, while remaining CompileList follow-up is narrowed to any still-uncharacterized non-CompileError user-surface cases beyond the landed split compile-error feedback handling, malformed `ScriptCompilerFlags`, custom `--nwscript` parse/path failures, and the managed-compiler-unavailable distinction; backup/uninstall semantics still need branch-local code decisions or follow-up fixes.
- Next-step change: continue the Delphi-first comparison on any remaining HACKList replace-file or manual-source runtime/user-surface edge cases now that non-replace destination behavior, missing-source failure semantics, and save-as copy/skip note wording are aligned, and only reopen CompileList if another concrete non-CompileError user-surface mismatch turns up beyond the already landed managed-compiler slice; keep backup/uninstall behavior outside the active slice unless a direct patch-result defect is observed.
