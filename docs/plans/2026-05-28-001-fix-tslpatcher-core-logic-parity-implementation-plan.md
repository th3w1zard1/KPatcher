...
- Landed: additional owner-path parity fixes now cover the binary-verified install queue order, vendor-default HACK write semantics within the existing NCS-backed surface, HACKList skip-on-existing behavior for compiled-script patches unless replacement is explicitly enabled, CompileList prep/include/failure behavior, removal of unsupported top-level `!DefaultSourceFolder` handling in CompileList, the vendored Override/source-name skip guard for non-replace CompileList entries, managed parsing/application of vendored `ScriptCompilerFlags` for CompileList under the repo's managed-compiler policy, SSF recovery plus 40-slot handling, vendored `!OverrideType` default/rename behavior, and removal of unsupported top-level `!DefaultDestination` handling outside CompileList.
- Partial/uncertain: remaining HACKList follow-up is now limited to broader source/destination edge cases, while remaining CompileList follow-up is narrowed to vendored compiler-runtime semantics beyond the landed source-name guard and managed `ScriptCompilerFlags` handling (notably compiler-missing / feedback / working-directory edge behavior); backup/uninstall semantics still need branch-local code decisions or follow-up fixes.
- Next-step change: keep the Delphi-first comparison on CompileList, specifically the remaining compiler-runtime semantics that are still visible in `DoCompileFiles()` after landing managed `ScriptCompilerFlags` parity, while leaving backup/uninstall behavior outside the active slice unless a direct patch-result defect is observed.
