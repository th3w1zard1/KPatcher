...
- Next-step change: keep the Delphi-first comparison on CompileList, specifically `DoCompileFiles()` around explicit compiler-availability behavior and any remaining user-surface wording/ordering differences after landing managed flag handling, non-recursive include lookup, and the currently characterized missing-include plus entry-point exception-feedback logging, while leaving backup/uninstall behavior outside the active slice unless a direct patch-result defect is observed.
