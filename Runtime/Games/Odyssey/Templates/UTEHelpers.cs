...
{
    // Moved from Andastra.Formats.Resource.Generics.UTEHelpers to Odyssey.Engines.Odyssey.Templates
    ...
    /// <remarks>
    /// UTE Helper Functions:
    /// - Based on swkotor2.exe encounter template loading/saving system
    /// - Located via string references: "Encounter" @ 0x007bc544, "EncounterList" @ 0x007c0c80, "Encounter template '%s' doesn't exist.\n" @ 0x007bf78c
    /// - Encounter loading: FUN_005223a0 @ 0x005223a0 loads encounter from GFF (construct_ute equivalent)
    /// - Encounter saving: FUN_005226d0 @ 0x005226d0 saves encounter to GFF (dismantle_ute equivalent)
    /// - ConstructUte: Parses GFF structure into UTE object, extracts all fields (creature spawns, difficulty, respawn settings, script hooks)
    /// - DismantleUte: Converts UTE object back to GFF structure, writes all fields in correct format
    /// - GFF field names match original engine exactly (TemplateResRef, Tag, Active, DifficultyIndex, Difficulty, Faction, MaxCreatures, etc.)
    /// - Creature spawns: CreatureList contains creature spawn definitions with ResRef, Appearance, CR (float), SingleSpawn, GuaranteedCount (K2 only)
    /// - Script hook fields: OnEntered, OnExit, OnExhausted, OnHeartbeat, OnUserDefined
    /// - Matching PyKotor implementation at Libraries/PyKotor/src/pykotor/resource/generics/ute.py:235-322
    /// </remarks>
