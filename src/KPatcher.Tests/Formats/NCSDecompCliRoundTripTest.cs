...
// managed NCSDecomp.Core decompile, Java-equivalent text normalization (warning-only),
// recompile with nwnnsscomp, raw bytecode equality when recompilation succeeds.
...
namespace KPatcher.Core.Tests.Formats
{
    ...
    /// <item><b>Allowed (comparison only):</b> whitespace and formatting normalization for text comparison (does not legitimize fixing bugs via normalization).</item>
    /// <item>If decompiled output differs from the original (other than formatting): bytecode check is primary; text mismatch is warning-only when the flow matches Java.</item>
    ...
    public sealed class NCSDecompCliRoundTripTest
    {
        ...
        private sealed class SourceCompilationException : Exception
        {
            public SourceCompilationException(string message, Exception cause)
                : base(message, cause)
            {
            }
        }
        ...
        private static void RoundTripSingle(string nssPath, string gameFlag, string scratchRoot)
        {
            ...
            try
            {
                ...
                long compareTime = Stopwatch.GetElapsedTime(compareTextStart).Ticks * 100;
                ...
                catch (Exception e)
                {
                    ...
                    if (textMismatchMessage != null)
                    {
                        throw new InvalidOperationException(
                            "Bytecode mismatch (PRIMARY FAILURE):\n" + e.Message + "\n\nAlso had text mismatch (secondary):\n" + textMismatchMessage);
                    }
        ...
        private static void RunCompiler(string originalNssPath, string compiledOut, string gameFlag, string workDir)
        {
            ...
            try
            {
                ...
                bool isK2 = string.Equals(gameFlag, "k2", StringComparison.Ordinal);
                var config = new NwnnsscompConfig(NwnCompiler, stagedSource, compiledOut, isK2);
        ...
        private static void AssertBytecodeEqual(string originalNcs, string roundTripNcs, string gameFlag, string displayName)
        {
            ...
            string decompiledNss = Path.Combine(
                Path.GetDirectoryName(roundTripNcs) ?? ".",
                Path.GetFileName(roundTripNcs).Replace(".rt.ncs", ".dec.nss", StringComparison.OrdinalIgnoreCase));
            bool decompiledExists = File.Exists(decompiledNss);
        ...
        private static void DecompileNcsToPcode(string ncsPath, string outputPcode, string gameFlag)
        {
            ...
            bool isK2 = string.Equals(gameFlag, "k2", StringComparison.Ordinal);
            var config = new NwnnsscompConfig(NwnCompiler, ncsPath, outputPcode, isK2);
