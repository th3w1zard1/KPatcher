        ...
        private void LoadRtfContent()
        {
            ...
            catch (Exception ex)
            {
                ...
                try
                {
                    ...
                    viewModel.AddLogEntry(stripped, Andastra.Formats.Logger.LogType.Note);
                }
                catch (Exception stripEx)
                {
                    Console.WriteLine($"[RTF] ERROR: Failed to strip RTF: {stripEx.Message}");
                    viewModel.IsRtfContent = false;
                    viewModel.AddLogEntry("Failed to load RTF content. Please check the console for details.", Andastra.Formats.Logger.LogType.Error);
                }
            }
        }
