...
namespace KPatcher.Core.Mods
{
    ...
    public class KPatcherINISerializer
    {
        ...
        private List<string> SerializeTlkList(List<ModificationsTLK> modifications, bool verbose)
        {
            ...
            // Modern KPatcher syntax for appends: Use StrRef token mappings ONLY
        ...
        private List<string> SerializeGffFile(ModificationsGFF modGff)
        {
            ...
            foreach (var gffModifier in modGff.Modifiers)
            {
                ...
                {
                    // Direct field modification: Path=Value (use backslashes per KPatcher)
        ...
        private string SerializeTokenUsage(object tokenUsage)
        {
            ...
            if (tokenUsage is global::KPatcher.Core.Memory.TokenUsageTLK tlkUsage)
            {
                return $"StrRef{tlkUsage.TokenId}";
            }
            if (tokenUsage is global::KPatcher.Core.Memory.TokenUsage2DA twodaUsage)
            {
                return $"2DAMEMORY{twodaUsage.TokenId}";
            }
            if (tokenUsage is global::KPatcher.Core.Memory.NoTokenUsage noToken)
            {
