...
{
    // Moved from AuroraEngine.Common.Resource.Generics.UTDHelpers to Odyssey.Engines.Odyssey.Templates
    ...
    /// <remarks>
    /// UTD Helper Functions:
    /// - Based on swkotor2.exe door template loading/saving system
    ...
    /// - ConstructUtd: Parses GFF structure into UTD object, extracts all fields (lock mechanics, HP, script hooks, appearance)
    /// - DismantleUtd: Converts UTD object back to GFF structure, writes all fields in correct format
    /// - GFF field names match original engine exactly (TemplateResRef, Tag, LocName, Conversation, Faction, etc.)
    /// - Lock mechanics: Lockable, Locked, KeyRequired, KeyName, OpenLockDC, OpenLockDiff, OpenLockDiffMod, OpenState
    /// - HP system: HP, CurrentHP, Hardness, Fort, Ref, Will
    /// - Script hook fields: OnClick, OnClosed, OnDamaged, OnDeath, OnOpenFailed, OnHeartbeat, OnMeleeAttacked, OnOpen, OnUnlock, OnLock, OnSpellCastAt
    /// - Matching PyKotor implementation at Libraries/PyKotor/src/pykotor/resource/generics/utd.py:498-665
    /// </remarks>
