...
{
    // Moved from AuroraEngine.Common.Resource.Generics.UTPHelpers to Odyssey.Engines.Odyssey.Templates
    ...
    /// - Based on swkotor2.exe placeable template loading/saving system
    /// - Located via string references: "Placeable" @ 0x007bc544, "PlaceableList" @ 0x007c0c80, "Placeable template '%s' doesn't exist.\n" @ 0x007bf78c
    /// - Placeable loading: FUN_005223a0 @ 0x005223a0 loads placeable from GFF (construct_utp equivalent)
    /// - Placeable saving: FUN_005226d0 @ 0x005226d0 saves placeable to GFF (dismantle_utp equivalent)
    /// - ConstructUtp: Parses GFF structure into UTP object, extracts all fields (lock mechanics, HP, inventory, script hooks, appearance)
    /// - DismantleUtp: Converts UTP object back to GFF structure, writes all fields in correct format
    /// - GFF field names match original engine exactly (TemplateResRef, Tag, LocName, Conversation, Faction, Appearance, etc.)
    /// - Lock mechanics: Lockable, Locked, KeyRequired, KeyName, OpenLockDC, OpenLockDiff, OpenLockDiffMod
    /// - HP system: HP, CurrentHP, Hardness, Fort, Ref, Will
    /// - Inventory: ItemList contains inventory items with InventoryRes, Repos_PosX, Repos_PosY, Dropable
    /// - Script hook fields: OnClick, OnClosed, OnDamaged, OnDeath, OnEndDialogue, OnFailToOpen, OnHeartbeat, OnInvDisturbed, OnMeleeAttacked, OnOpen, OnUnlock, OnUsed, OnLock, OnSpellCastAt
    /// - Matching PyKotor implementation at Libraries/PyKotor/src/pykotor/resource/generics/utp.py:185-377
    /// </remarks>
