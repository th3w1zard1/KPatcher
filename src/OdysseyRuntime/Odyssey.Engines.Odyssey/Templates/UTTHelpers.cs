...
{
    // Moved from AuroraEngine.Common.Resource.Generics.UTTHelpers to Odyssey.Engines.Odyssey.Templates
    ...
    /// <remarks>
    /// UTT Helper Functions:
    /// - Based on swkotor2.exe trigger template loading/saving system
    /// - Located via string references: "Trigger" @ 0x007bc544, "TriggerList" @ 0x007c0c80, "Trigger template '%s' doesn't exist.\n" @ 0x007bf78c
    /// - Trigger loading: FUN_005223a0 @ 0x005223a0 loads trigger from GFF (construct_utt equivalent)
    /// - Trigger saving: FUN_005226d0 @ 0x005226d0 saves trigger to GFF (dismantle_utt equivalent)
    /// - ConstructUtt: Parses GFF structure into UTT object, extracts all fields (trap mechanics, script hooks, activation settings)
    /// - DismantleUtt: Converts UTT object back to GFF structure, writes all fields in correct format
    /// - GFF field names match original engine exactly (TemplateResRef, Tag, AutoRemoveKey, Faction, Cursor, HighlightHeight, etc.)
    /// - Trap mechanics: TrapDetectable, TrapDetectDC, TrapDisarmable, DisarmDC, TrapFlag, TrapOneShot, TrapType
    /// - Script hook fields: OnDisarm, OnTrapTriggered, OnClick, ScriptHeartbeat, ScriptOnEnter, ScriptOnExit, ScriptUserDefine
    /// - Matching PyKotor implementation at Libraries/PyKotor/src/pykotor/resource/generics/utt.py:212-324
    /// </remarks>
