...
{
    // Moved from AuroraEngine.Common.Resource.Generics.UTSHelpers to Odyssey.Engines.Odyssey.Templates
    ...
    /// <remarks>
    /// UTS Helper Functions:
    /// - Based on swkotor2.exe sound template loading/saving system
    ...
    /// - ConstructUts: Parses GFF structure into UTS object, extracts all fields (audio settings, positioning, looping, volume controls)
    /// - DismantleUts: Converts UTS object back to GFF structure, writes all fields in correct format
    /// - GFF field names match original engine exactly (TemplateResRef, Tag, LocName, Active, Continuous, Looping, Positional, etc.)
    /// - Audio settings: Volume, VolumeVrtn, PitchVariation, Elevation, MinDistance, MaxDistance, DistanceCutoff, Priority
    /// - Timing: Hours, Times, Interval (UInt32), IntervalVrtn (UInt32)
    /// - Sound list: Sounds contains list of ResRef sound files
    /// - Matching PyKotor implementation at Libraries/PyKotor/src/pykotor/resource/generics/uts.py:199-311
    /// </remarks>
