...
{
    // Moved from AuroraEngine.Common.Resource.Generics.UTMHelpers to Odyssey.Engines.Odyssey.Templates
    ...
    /// - Based on swkotor2.exe merchant template loading/saving system
    ...
    /// - ConstructUtm: Parses GFF structure into UTM object, extracts all fields (inventory, pricing, script references)
    /// - DismantleUtm: Converts UTM object back to GFF structure, writes all fields in correct format
    /// - GFF field names match original engine exactly (ResRef, LocName, Tag, MarkUp, MarkDown, BuySellFlag, OnOpenStore, etc.)
    /// - Pricing: MarkUp, MarkDown for buy/sell price modifiers
    /// - Buy/Sell flags: BuySellFlag (UInt8) - bit 0 = can_buy, bit 1 = can_sell
    /// - Inventory: ItemList contains merchant inventory items with InventoryRes, Infinite, Dropable, Repos_PosX, Repos_PosY
    /// - Script hooks: OnOpenStore for store opening script
    /// - Matching PyKotor implementation at Libraries/PyKotor/src/pykotor/resource/generics/utm.py:130-223
    /// </remarks>
