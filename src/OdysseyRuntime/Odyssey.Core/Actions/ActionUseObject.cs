...
namespace Odyssey.Core.Actions
{
    ...
    public class ActionUseObject : ActionBase
    {
        ...
        /// <summary>
        /// Gets the use point for an object, preferring BWM hook positions if available.
        /// Based on swkotor2.exe: Objects can have USE1/USE2 hook vectors from their walkmesh (DWK/PWK)
        /// Hook vectors define interaction points where the player should stand to use the object
        /// </summary>
        private Vector3 GetUsePoint(IEntity target, ITransformComponent targetTransform)
        {
            // Check if entity has stored BWM hook positions (loaded from walkmesh)
            // Hook positions are stored as "BWMHook1" and "BWMHook2" in entity data
            // These are absolute world positions (position + relative hook)
            if (target is Core.Entities.Entity entity)
            {
                // Try to get stored hook positions
                Vector3? hook1 = entity.GetData<Vector3?>("BWMHook1");
                Vector3? hook2 = entity.GetData<Vector3?>("BWMHook2");
                
                if (hook1.HasValue || hook2.HasValue)
                {
                    // Use the first available hook, or closest to actor if both available
                    // For now, prefer hook1 (USE1) as it's typically the primary interaction point
                    if (hook1.HasValue)
                    {
                        return hook1.Value;
                    }
            ...
            // Fallback: Use object position if no hooks available
            // TODO: Load BWM hooks from door/placeable walkmesh files (DWK/PWK) during template loading
            // BWM format stores RelativeHook1/RelativeHook2 and AbsoluteHook1/AbsoluteHook2
            // These should be loaded from the object's model walkmesh and stored in entity data
